# Marginalia 桌面 GUI 使用教程（教师版）

这份教程面向没有编程经验的学校老师。目标是：安装后知道先配置什么、每个设置项是什么意思、哪些保持默认即可、什么时候才需要本地模型、嵌入模型和重排模型。
...
5. 普通老师最少只需要配置 `Default` 这一个 LLM 配置。`chat`、`reflect`、`ingest` 留空即可继承 `Default`。
...
导入后后台会做分析任务。长文档会自动分块，不需要老师提前处理。等状态变为完成后，再去聊天页提问效果最好。
...
适合大多数学校电脑。供应商只要提供 OpenAI-compatible 接口即可。
...
| Concurrent ingest tasks | 后台同时分析多少个文件任务。 | 学校普通电脑 `3-5`；本地模型 `1-2`；稳定云 API 可用 `10`。 |
...
| Worker | 后台任务是否启用。 | 启用。 |
| Auto lifecycle | 是否自动把文件降级/归档。 | 学校和个人资料库建议关闭，手动管理。 |
...
2. 程序不会在启动时自动联网检查版本。
3. 校园网络如果无法访问 GitHub，检查失败是正常的，可以手动去项目 Releases 页面下载。
